...
/**
 * 추상화 연습하기
 * 1. 로그인 시스템 연습하기 - 인증 방식에 따른 사용가 가입
 */

const user = [{name: 'tjalsdud', pwd: '1234'}];
const emailUser = {name: 'tjalsdud@naver.com', pwd: '12423'};

const input = ({id, pwd}) => _.map((id,pwd) => {id, pwd});
const check = (f) => _.filter(info => f(info))
const auth = (chf, f ) => _.pipe(_.map(info => chf(info)),_.filter(info => f(info)));

const sign = (info, checkf,  chf, authf) => _.go(info, input, check(checkf), auth(chf, authf), _.log);
sign(user, (info) => info.pwd === '1234', (info) => info.pwd + info.name, (authData) => authData.length > 4 );
